...
extern int32 rtl_get_hw_fdb_port(uint32 fid, ether_addr_t *mac, uint32 flags,
	uint8 *port);
...
extern unsigned int (*rtl_init_bypass_packet_hook) (struct Path_List_Entry *
	entry_path);
extern unsigned int (*rtl_check_bypass_packet_hook) (struct Path_List_Entry *
	entry_path);
...
extern int proc_hwnat_packet_bypassed_tcpport_read_proc(char *page,
	char **start, off_t off, int count, int *eof, void *data);
extern int proc_hwnat_packet_bypassed_tcpport_write_proc(struct file *file,
	const char *buffer, unsigned long count, void *data);
extern int proc_hwnat_packet_bypassed_udpport_read_proc(char *page,
	char **start, off_t off, int count, int *eof, void *data);
extern int proc_hwnat_packet_bypassed_udpport_write_proc(struct file *file,
	const char *buffer, unsigned long count, void *data);
#endif
#include <linux/version.h>
...
{
	__be32 ip = queryIP;
	struct neighbour *neigh, *n;
	int err = -ENXIO, irqs_disabled_status = 1;
	...
	this_cpu_inc((&arp_tbl)->stats->lookups);

	if(likely(irqs_disabled_status))
		rcu_read_lock();
	else
		rcu_read_lock_bh();

	n = __neigh_lookup_noref(&arp_tbl, &ip, dev);
	if (n) {
...
#if defined(CONFIG_RTD_1295_HWNAT)
static int ext_service_ports_read_proc(struct seq_file *s, void *v)
{
	int i;

	for (i = 0; i < MAX_EXT_SERVICE_PORTS; i++) {
		if (ext_service_ports[i]) {
			seq_printf(s, "%u ", ext_service_ports[i]);
		}
	}
...
static int read_proc(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "fastpath %s, GC_RX_Count %d, Status: %d\n",
		...
		rtl_fp_gc_rx_threshold, rtl_newGC_session_status_flags);
#else
...
static ssize_t fastpath_single_write(struct file *file,
...
static ssize_t ext_service_ports_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
static ssize_t fastspi_single_write(struct file *file,
	const char __user *userbuf, size_t count, loff_t *off)
{
...
int rtl_ip_fragment_for_fastpath(struct sk_buff *skb,
	...
	rtl865x_qos_mark *qosMark, struct sk_buff *pskb)
{
...
#endif
	{
		...
		memset(resMac, 0, 14);
		if ((lanDev != NULL) &&
			(arp_req_get_ha(naptEntry->intIp, lanDev,
					resMac) == 0)) {
			...
			memcpy(eth_hdr(pskb)->h_dest, lanDev->dev_addr,
				ETH_ALEN);
		}
		...
		if (proto == ETH_P_IP) {
			...
			#else /* CONFIG_RTD_1295_HWNAT */
			(list_empty(&nf_hooks[PF_INET][NF_IP_PRE_ROUTING])) ? :
				ipt_do_table(pskb, NF_IP_PRE_ROUTING, lanDev,
				wanDev, dev_net(lanDev)->ipv4.iptable_mangle);
			#endif /* CONFIG_RTD_1295_HWNAT */
		}
		...
		memset(resMac, 0, 14);
		if ((wanDev != NULL) &&
			(arp_req_get_ha(naptEntry->remIp, wanDev,
					resMac) == 0)) {
			/* Patch for pppoe wantype: run udp chariot */
			/* bak skb mac header */
			if ((memcmp(pskb->data - 2, pppProto, 2) == 0)	/* equal 0x0021 */
			...
			memcpy(eth_hdr(pskb)->h_source, wanDev->dev_addr,
				ETH_ALEN);
		}
		...
		if (proto == ETH_P_IP) {
			...
			#else /* CONFIG_RTD_1295_HWNAT */
			(list_empty(&nf_hooks[PF_INET][NF_IP_POST_ROUTING])) ? :
				ipt_do_table(pskb, NF_IP_POST_ROUTING, lanDev,
				wanDev, dev_net(wanDev)->ipv4.iptable_mangle);
			#endif /* CONFIG_RTD_1295_HWNAT */
		}
		DEBUGP_API("[%s][%d]:[%s][%s][%s][%s][%d]\n", __FUNCTION__,
			__LINE__, lanDev ? lanDev->name : "NULL",
...
#endif
	{
		...
		if ((wanDev != NULL) &&
			...
			/* Replace source mac addr to check uplink mark */
			memcpy(eth_hdr(pskb)->h_source, resMac, ETH_ALEN);
			memcpy(eth_hdr(pskb)->h_dest, wanDev->dev_addr,
				ETH_ALEN);
		}
		...
		if (proto == ETH_P_IP) {
			...
			#else /* CONFIG_RTD_1295_HWNAT */
			(list_empty(&nf_hooks[PF_INET][NF_IP_PRE_ROUTING])) ? :
				ipt_do_table(pskb, NF_IP_PRE_ROUTING, wanDev,
				lanDev, dev_net(wanDev)->ipv4.iptable_mangle);
			#endif /* CONFIG_RTD_1295_HWNAT */
		}
		DEBUGP_API("[%s][%d]:[%s][%s][%s][%s][%d]\n", __FUNCTION__,
			__LINE__, lanDev ? lanDev->name : "NULL",
		...
		if ((lanDev != NULL) &&
			...
			memcpy(eth_hdr(pskb)->h_dest, resMac, ETH_ALEN);
			memcpy(eth_hdr(pskb)->h_source, lanDev->dev_addr,
				ETH_ALEN);
		}
		pskb->mark = 0;	/* initial */
#if defined(CONFIG_RTL_SW_QUEUE_DECISION_PRIORITY)
		...
		if (proto == ETH_P_IP) {
			...
			#else /* CONFIG_RTD_1295_HWNAT */
			(list_empty(&nf_hooks[PF_INET][NF_IP_POST_ROUTING])) ? :
				ipt_do_table(pskb, NF_IP_POST_ROUTING, wanDev,
				lanDev, dev_net(lanDev)->ipv4.iptable_mangle);
			#endif /* CONFIG_RTD_1295_HWNAT */
		}
		DEBUGP_API("[%s][%d]:[%s][%s][%s][%s][%d]\n", __FUNCTION__,
			__LINE__, lanDev ? lanDev->name : "NULL",
...
#if defined(CONFIG_RTL_SW_QUEUE_DECISION_PRIORITY)
	qosMark[port_decision_priority].downlinkMark =
		(postMarkExt[port_decision_priority] ?
		postMarkExt[port_decision_priority] :
		preMarkExt[port_decision_priority]);
	qosMark[vlan_decision_priority].downlinkMark =
		(postMarkExt[vlan_decision_priority] ?
		postMarkExt[vlan_decision_priority] :
		preMarkExt[vlan_decision_priority]);
	qosMark[dscp_decision_priority].downlinkMark =
		(postMarkExt[dscp_decision_priority] ?
		postMarkExt[dscp_decision_priority] :
		preMarkExt[dscp_decision_priority]);
	...
	uint32 *fp_policy_route_mark)
{
	...
	if (*fp_policy_route_mark == 0) {
		...
		if ((strncmp(skb->dev->name, RTL_DRV_ETHLAN_P0_NETIF_NAME,
					strlen(RTL_DRV_ETHLAN_P0_NETIF_NAME)) == 0) ||
			(strncmp(skb->dev->name, RTL_DRV_LAN_P1_NETIF_NAME,
					strlen(RTL_DRV_LAN_P1_NETIF_NAME)) == 0)
			|| (strncmp(skb->dev->name, RTL_DRV_LAN_P2_NETIF_NAME,
					strlen(RTL_DRV_LAN_P2_NETIF_NAME)) == 0)
			|| (strncmp(skb->dev->name, RTL_DRV_LAN_P3_NETIF_NAME,
					strlen(RTL_DRV_LAN_P3_NETIF_NAME)) == 0)
			|| (strncmp(skb->dev->name, "wlan",
					strlen("wlan")) == 0)) {
			...
			(list_empty(&nf_hooks[PF_INET][NF_INET_PRE_ROUTING])) ? :
				rtl_ipt_do_table(skb, NF_INET_PRE_ROUTING,
				lanDev, wanDev);
			*fp_policy_route_mark = skb->mark;
#else
			(list_empty(&nf_hooks[PF_INET][NF_IP_PRE_ROUTING])) ? :
				rtl_ipt_do_table(skb, NF_IP_PRE_ROUTING, lanDev,
				wanDev);
			*fp_policy_route_mark = skb->mark;
#endif
		}
...
static inline void enter_fast_path_fast_l2tp_pre_process(struct sk_buff *skb)
{
	...
	if (fast_l2tp_fw) {
		...
		if (skbIn_dev == NULL) {
			if ((skbNetDevice = __dev_get_by_name(&init_net,
						l2tprx_dev->name)) != NULL) {
				if ((skbIn_dev = (struct in_device *)
						skbNetDevice->ip_ptr) != NULL)
...
#if defined(CONFIG_RTL_NF_CONNTRACK_GARBAGE_NEW)
static inline int rtl_fp_gc_status_check_priority(uint32 sIp, uint32 dIp,
...
static inline int rtl_fp_gc_status_check(struct iphdr *iph, struct tcphdr *tcph,
	struct sk_buff *skb)
{
	uint32 sIp, dIp;
	uint16 sPort, dPort;
	...
	if ((rtl_newGC_session_status_flags != RTL_FP_SESSION_LEVEL_IDLE) &&
		time_after_eq(rtl_newGC_session_status_time, jiffies)) {
		sIp = iph->saddr;
		dIp = iph->daddr;
		sPort = tcph->source;
		dPort = tcph->dest;
		if (rtl_fp_gc_status_check_priority(sIp, dIp, sPort,
				dPort) == NET_RX_DROP) {
			kfree_skb(skb);
			ret = NET_RX_DROP;
		}
		...
		if (rtl_newGC_session_status_flags == RTL_FP_SESSION_LEVEL3) {
			if ((rx_count++) > rtl_fp_gc_rx_threshold) {
				...
				rtl_newGC_session_status_time =
					jiffies + RTL_FP_SESSION_LEVEL3_INTERVAL;
			}
			...
			if ((rx_count) > rtl_fp_gc_rx_threshold) {
				rx_count = 0;
				rtl_newGC_session_status_time =
					jiffies + RTL_FP_SESSION_LEVEL1_INTERVAL;
			}
		}
	}
...
{
#if	defined(FAST_L2TP)
	if (fast_l2tp_fw && skb->dev &&
		(!memcmp(skb->dev->name, RTL_FASTPATH_PPP0_DEV_NAME, 4))) {
#if defined(RTL_FASTPATH_FRAGMENT_SUPPORT)
		if (skb->len > dst_mtu(skb->dst)) {
			if (fast_l2tp_to_wan_check(skb) == 0)
				return NET_RX_SUCCESS;
			/*Acturally, rtl_ip_fragment_for_fastpath will never return 0. */
			if (rtl_ip_fragment_for_fastpath(skb,
					fast_l2tp_to_wan2))
				return NET_RX_DROP;
		} else
#endif
	...
	void *out)
{
	...
	#else /* CONFIG_RTD_1295_HWNAT */
	return ipt_do_table(skb, hook, in_dev, out_dev,
		dev_net(skb->dev)->ipv4.iptable_mangle);
	#endif /* CONFIG_RTD_1295_HWNAT */
}
	...
	int is_wan)
{
	...
	if (gQosEnabled) {
		if (is_wan == 0) {
			if (rtl_get_hw_fdb_port(RTL_LAN_FID,
					(ether_addr_t *)resMac, FDB_DYNAMIC,
					&phyPort) == SUCCESS) {
				rtl_set_skb_phy_port(skb, phyPort);
			}
		} else if (is_wan == 1) {
			if (rtl_get_hw_fdb_port(RTL_WAN_FID,
					(ether_addr_t *)resMac, FDB_DYNAMIC,
					&phyPort) == SUCCESS) {
				rtl_set_skb_phy_port(skb, phyPort);
			}
...
void rtl_set_fdb_aging(void *fdb_ptr, unsigned long value)
{
	struct net_bridge_fdb_entry *fdb =
		(struct net_bridge_fdb_entry *)fdb_ptr;
...
void rtl_set_iph_dscp(struct Path_List_Entry *entry_path, struct iphdr *iph)
{
	...
	if ((entry_path->dscp_in != 0) && (entry_path->course == 2))	/* in bound flow */
	{
		...
		FASTPATH_ADJUST_CHKSUM_NAT(ntohl(*(uint32 *)iph),
			ntohl(iph_firstword_bf), iph->check);
	}
...
{
#if defined(CONFIG_RTL_NF_CONNTRACK_GARBAGE_NEW)
	if (ct_tmp->drop_flag == -1 && (ipprotocol == IPPROTO_TCP ||
...
int rtl_Add_Pattern_ACL_For_ContentFilter(void)
{
...
#if defined(CONFIG_RTL_NETIF_MAPPING)
	{
		...
		rtl865x_add_pattern_acl_for_contentFilter(&rule,
			entry->drvName);
	}
#else
...
int Del_Pattern_ACL_For_ContentFilter(void)
{
...
#if defined(CONFIG_RTL_NETIF_MAPPING)
	{
		...
		rtl865x_del_pattern_acl_for_contentFilter(&rule,
			entry->drvName);
	}
#else
	...
	struct in6_addr *addr)
{
	...
	if (flag == 0)
		memcmp(addr, &ct->tuplehash[dir].tuple.src.u3.in6,
			sizeof(struct in6_addr));
	else if (flag == 1)
		memcmp(addr, &ct->tuplehash[dir].tuple.dst.u3.in6,
			sizeof(struct in6_addr));
}
	...
	void *out)
{
	...
	ret = ip6t_do_table(skb, hook, in_dev, out_dev,
		dev_net(skb->dev)->ipv6.ip6table_mangle);
	return ret;
}
	...
	uint32 iphProtocol)
{
	...
	if (iphProtocol == IPPROTO_TCP) {
		...
		ret = rtl_nf_conntrack_in(dev_net(skb->dev), dataoff,
...
int _ip_finish_output3(struct sk_buff *skb)
{
	...
	if (gQosEnabled) {
		...
		if (proto == ETH_P_IP) {
			...
			state.out = NULL;
			(list_empty(&nf_hooks[PF_INET][NF_IP_POST_ROUTING])) ? :
				ipt_do_table(skb, &state,
				dev_net(skb->dev)->ipv4.iptable_mangle);
			#else /* CONFIG_RTD_1295_HWNAT */
			(list_empty(&nf_hooks[PF_INET][NF_IP_POST_ROUTING])) ? :
				ipt_do_table(skb, NF_IP_POST_ROUTING, skb->dev,
				NULL, dev_net(skb->dev)->ipv4.iptable_mangle);
			#endif /* CONFIG_RTD_1295_HWNAT */
		}
	}
#endif
...
#else
		memcpy((unsigned char *)UNCACHE(skb->data - 16), hh->hh_data,
			16);
#endif
		...
		if (skb->dev->flags & IFF_UP) {
...
#endif
			{
				...
				if (skb->dev->priv_flags == IFF_EBRIDGE) {
					...
					vid = 0;
#endif
					if ((dst = __br_fdb_get(br, dest,
								vid)) != NULL)
#else
					if ((dst = __br_fdb_get(br,
								dest)) != NULL)
#endif
					{
...
/* ------------------------------- david+2007-05-25 */
	...
	if (net_ratelimit())
		printk(KERN_DEBUG
			"ip_finish_output3: No header cache and no neighbour!\n");
#endif
	kfree_skb(skb);
	return -EINVAL;
}
...
int FastPath_Enter(struct sk_buff **pskb)
{
	...
	if (fast_pppoe_fw) {
		if ((strncmp(skb->dev->name, RTL_PS_PPP_NAME,
					strlen(RTL_PS_PPP_NAME)) == 0) &&
...
/* bug fix:when port filter is enable,application will disable fast_nat_fw,at this moment,url filter is abnormal... */
	...
	if (upnp_br_enabled && skb->dev &&
		!memcmp(skb->dev->name, RTL_PS_WAN0_DEV_NAME, 4) &&
		filter_upnp_and_fw(skb)) {
		return 1;
	}
#endif
	...
	if (ret != NET_RX_DROP) {
		...
		if (ip_hdr(skb)->protocol == IPPROTO_TCP) {
			tcpudph = (struct tcphdr *)((__u32 *) skb->data +
				ip_hdr(skb)->ihl);
...
#endif
#ifdef FAST_PPTP
	if (fast_pptp_fw && ret == 0 && ip_hdr(skb)->protocol == IPPROTO_GRE &&
		skb->len > sizeof(struct iphdr) && pptp_tcp_finished == 1)
...
static int proc_packet_bypassed_write_proc(struct file *file,
	const char *buffer, unsigned long count, void *data)
{
	if (count < 1)
		return -EFAULT;
	...
	if (buffer && !copy_from_user(packet_bypassed_buffer, buffer, count)) {
		sscanf(packet_bypassed_buffer, "%d",
			&fastpath_packet_bypassed_num);
	}
	if (packet_bypassed_buffer)
...
static int __init fastpath_init(void)
{
	...
	proc_packet_bypassed =
		create_proc_entry("fastpath_packet_bypassed_num", 0, NULL);
	if (proc_packet_bypassed) {
		proc_packet_bypassed->read_proc =
			proc_packet_bypassed_read_proc;
		proc_packet_bypassed->write_proc =
			proc_packet_bypassed_write_proc;
	}

	proc_packet_bypassed_tcpport =
		create_proc_entry("hwnat_packet_bypassed_tcpport_set", 0, NULL);
	if (proc_packet_bypassed_tcpport) {
		proc_packet_bypassed_tcpport->read_proc =
			proc_hwnat_packet_bypassed_tcpport_read_proc;
		proc_packet_bypassed_tcpport->write_proc =
			proc_hwnat_packet_bypassed_tcpport_write_proc;
	}

	proc_packet_bypassed_udpport =
		create_proc_entry("hwnat_packet_bypassed_udpport_set", 0, NULL);
	if (proc_packet_bypassed_udpport) {
		proc_packet_bypassed_udpport->read_proc =
			proc_hwnat_packet_bypassed_udpport_read_proc;
		proc_packet_bypassed_udpport->write_proc =
			proc_hwnat_packet_bypassed_udpport_write_proc;
	}
#else
...
int Direct_Send_Reply(struct sk_buff *skb, int offset)
{
	...
	if (!fast_l2tp_fw || l2tpInfo.tid == 0 || l2tpInfo.cid == 0 ||
		!l2tpInfo.wan_dev)
		return 0;
	...
	skb_push(skb, ETH_HLEN);	/* mac header push */
	{
		...
		iph_new->check = 0;
		iph_new->check =
			htons(ip_fast_csum((unsigned char *)iph_new,
				iph_new->ihl));
		...
		memcpy(skb->data + ETH_HLEN + sizeof(struct iphdr), &l2tphone,
			sizeof(struct Rus_l2tp_ext_hdr));
...
void rtl_fp_mark_invalid(void *ct)
{
	...
	mark_path_invalid(reply_sip, reply_sport, reply_dip, reply_dport,
		reply_proto);
