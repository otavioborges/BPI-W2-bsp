 ...
 */
static void iwl_bg_statistics_periodic(struct iwl_priv *priv)
{
	if (test_bit(STATUS_EXIT_PENDING, &priv->status))
		return;
 ...
 * if enabled
 */
static void iwl_bg_ucode_trace(struct iwl_priv *priv)
{
	if (test_bit(STATUS_EXIT_PENDING, &priv->status))
		return;
...
static void iwl_setup_deferred_work(struct iwl_priv *priv)
{
	...
	setup_timer(&priv->statistics_periodic, (void (*)(unsigned long))iwl_bg_statistics_periodic, (unsigned long)priv);

	setup_timer(&priv->ucode_trace, (void (*)(unsigned long))iwl_bg_ucode_trace, (unsigned long)priv);
}
